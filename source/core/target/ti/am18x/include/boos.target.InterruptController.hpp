...
namespace target
{
  ...
  public:
    ...
    virtual bool setHandler(::api::Task& handler, int32 source)
    {
      ...
      if(!isSource(source)) return global().enable(is, false);
  ...
  private:    
    ...
    static bool init(const Configuration& cfg)
    {
      ...
      do{
        ...
        // Stage 2 creates global interrupts class switching
        stage++;
        global_ = new Global();
